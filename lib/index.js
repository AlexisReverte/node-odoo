...
Odoo.prototype.connect = function (cb) {
  ...
  var req = http.request(options, function (res) {
    ...
    res.on('end', function () {
      ...
      for( var i = 0; res.headers['set-cookie'] && i < res.headers['set-cookie'].length; i++) {
        var cookieEntry = res.headers['set-cookie'][i];

        if( cookieEntry.indexOf('session_id') === 0 ) {
          self.sid = cookieEntry.split(';')[0];
        }
      }
...
// Get record
Odoo.prototype.get = function (model, id, params, callback) {
